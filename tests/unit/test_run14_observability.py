"""Regression tests for scripts/run14_observability.py per_model parsing.
...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "run14_observability.py"


def _import_observability_module():
    """Load scripts/run14_observability.py by path (scripts/ is not a package)."""
    import importlib.util
    spec = importlib.util.spec_from_file_location("run14_observability", SCRIPT_PATH)
