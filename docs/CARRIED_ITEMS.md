...
| **CI-q** | Production and certifiable callers still evaluate over unattributed populations. `ClinicalEvaluator.evaluate` is called inside the package without `source_id`, so those results carry no membership fingerprint and are not certifiable. | 3b-0 | at least one `.evaluate(` call site in `src/` passes no `source_id` |
