...
| **CI-p** | `MetricResult.to_dict` emits raw `NaN` while `from_dict` documents reading `null` back as `NaN`, and `dump_strict_json` refuses `NaN` by design. Normalised at the report layer in 3a. **RESCOPED 2026-07-28: the five Family B call sites are NOT persistence-reachable** — measured, no path carries `GeometrySummary`, `PartitionAgreementPanel`, `ConfounderComparison` or `ConfounderGate` output into strict JSON, so the blast radius this item claimed does not exist. What remains is the writer/reader asymmetry itself. | 3a | `to_dict()` on an UNDEFINED result returns a non-finite `value` |
| **CI-u** | The FLAT report surface cannot represent absence. `auroc` and `tpr_curve[0]` are `NaN` on a single-class cohort and `dump_strict_json` refuses them, so a scientifically valid evaluation over a degenerate cohort produces an artifact that **cannot be written at all**. Staged: **u-1 unify the writers (done)**, **u-2 the absence vocabulary (done)**, u-3 wire it into the report with a schema bump and read path. | CI-p investigation | `save_report` raises on a single-class cohort |
