#!/usr/bin/env python3
"""patch_register_drift_agents.py -- register the monitor agents in the Orchestrator.
...
AGENTS = [
    ...
    ("version_monitor_agent",               "VersionMonitorAgent"),
]

def fail(m): print(f"ABORT: {m}"); sys.exit(1)

def main() -> int:
    if not ORCH.exists(): fail(f"not found: {ORCH.resolve()}")
    txt = ORCH.read_text(encoding="utf-8")
    if txt.count(IMPORT_ANCHOR) != 1: fail(f"import anchor count != 1 ({txt.count(IMPORT_ANCHOR)})")
    if txt.count(REGISTRY_ANCHOR) != 1: fail(f"registry anchor count != 1 ({txt.count(REGISTRY_ANCHOR)})")
    ni, ne, skipped, added = "", "", [], []
    for module, cls in AGENTS:
        if cls in txt: skipped.append(cls); continue
        ni += f"        from genomic_variant_classifier.agent_layer.agents.{module} import {cls}\n"
        ne += f'            "{cls}": {cls},\n'
        added.append(cls)
    if not added:
        print(f"no-op: all {len(AGENTS)} agents already registered"); return 0
    patched = txt.replace(IMPORT_ANCHOR, IMPORT_ANCHOR + ni, 1)
    patched = patched.replace(REGISTRY_ANCHOR, REGISTRY_ANCHOR + ne, 1)
    bak = ORCH.with_suffix(".py.bak"); shutil.copy2(ORCH, bak)
    ORCH.write_text(patched, encoding="utf-8")
    try: py_compile.compile(str(ORCH), doraise=True)
    except py_compile.PyCompileError as e:
        shutil.copy2(bak, ORCH); fail(f"py_compile failed, reverted: {e}")
    print(f"registered ({len(added)}): {', '.join(added)}")
    if skipped: print(f"already present (skipped): {', '.join(skipped)}")
    print(f"backup: {bak}")
    return 0
