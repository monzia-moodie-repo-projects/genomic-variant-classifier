...
# =========================================================================
# G2 - On-VM ENVIRONMENT/HARDWARE pre-flight for Run 17 (env checks are run-agnostic; Charter v1.1 gate G2).
#
# Runs ON the Vast.ai instance AFTER SCP-up of the repo, BEFORE
# scripts/launch_run17_baseline.sh. This is the hardware+environment COMPLEMENT to
# launch_run17_baseline.sh's built-in data/code preflight, with NO overlap:
#   - launch_run17_baseline.sh owns: data-file presence (under $DATA), patch verify,
...
#   # exit 0 = environment green; proceed to launch_run17_baseline.sh
...
# PATH fix for vastai/pytorch images (mirror launch_run17_baseline.sh) so G2 checks the
...
if [[ "$FAIL_COUNT" -gt 0 ]]; then
  ...
  echo ""
  echo "DO NOT launch. Fix the above, then re-run G2 before launch_run17_baseline.sh."
  exit 1
fi
echo "G2 GREEN -- environment ready. Proceed to: bash scripts/launch_run17_baseline.sh"
exit 0