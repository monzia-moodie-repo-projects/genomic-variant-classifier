...
Usage — single chromosome VCF:
  python scripts/build_gnomad_parquet.py \\
      --vcf  data/external/gnomad/gnomad.exomes.v4.1.sites.chr1.vcf.bgz \\
      --out  data/processed/gnomad_v4_exomes.parquet

Usage — all chromosomes at once (glob):
  python scripts/build_gnomad_parquet.py \\
...
def parse_vcf(vcf_path: str) -> list[dict]:
    ...
    with _open_vcf(vcf_path) as fh:
        for line in fh:
            ...
            # Skip multi-allelic sites (comma in ALT) — gnomAD sites VCF is
            # already split, but guard anyway
            if "," in alt_field:
                skipped += 1
                continue
            ...
            rows.append({
                "variant_id":  f"{chrom}:{pos}:{ref}:{alt_field}",
                "allele_freq": af,
            })

    if skipped:
        logger.info("  Skipped %d multi-allelic lines in %s", skipped, vcf_path)
    return rows
...
def main() -> int:
    ...
    p.add_argument("--vcf", required=True,
                   help="Path or glob to gnomAD sites VCF(s) (.vcf / .vcf.gz / .vcf.bgz)")
    ...
    vcf_paths = sorted(glob.glob(args.vcf))
    if not vcf_paths:
        logger.error("No files matched: %s", args.vcf)
        return 1
    ...
    for vcf_path in vcf_paths:
        ...
        rows = parse_vcf(vcf_path)
        logger.info("  → %d variants", len(rows))
